        ...
        <div class="page-wrapper">
            ...
            <div class="container-fluid">
                ...
                <table class="table table-striped data-table" style="width: 100%; text-align:center;">
                    ...
                    <tbody>
                        <?php
                        $sql = "SELECT * FROM `purchased_products` where developer_name = '" . $_SESSION['username'] . "' ";
                        $records = mysqli_query($con, $sql); // fetch data from database
                        while ($data = mysqli_fetch_array($records)) {
                        ?>

                            <tr class="active-row" align="center">
                                <td id="soft_name"><?php echo $data['product_name'];
                                                    $status = 2; ?></td>
                                <td style="text-align:center ;"><?php echo $data['buyer_name']; ?></td>
                                <td style="text-align:center ;"><?php echo $data['product_group']; ?></td>
                ...
                <table class="table table-striped data-table" style="width: 100%; text-align:center;">
                    <h3 style="text-align:center;">Wallet Add Balance</h3>
                    <thead>
                        <tr>
                            <th>Order Id</th>
                            <th>Status</th>
                    ...
                    <tbody>
                        <?php
                        $sql = "SELECT * FROM `add_balance` where username = '" . $_SESSION['username'] . "' ";
                        $records = mysqli_query($con, $sql); // fetch data from database
                        while ($data = mysqli_fetch_array($records)) {
                        ?>

                            <tr class="active-row" align="center">
                                <td id="soft_name"><?php echo $data['order_id']; ?></td>

                                <!-- <span class="badge badge-pill badge-success">Success</span> -->
                                <?php
                                if ($data['status'] == 'pending') {
                                    echo ' <td>  <span class="badge badge-pill badge-warning">Pending</span> </td>';
                                    //    echo '<td style="text-align:center ;">' .  $data['order_created'] .'</td>';
                                    echo '<td> <form action="./cancel-transaction.php" method="post">
                                        <input type="hidden" name="oid" value="' . $data['order_id'] . '" >
                                        <input type="hidden" name="uname" value="' . $data['username'] . ' "><button type="submit" class="btn waves-effect waves-light btn-outline-danger">❌</button></form></td>';
                                } elseif ($data['status'] == 'failed') {
                                    echo ' <td> <span class="badge badge-pill badge-danger">Failed</span> </td>';
                                    //    echo '<td style="text-align:center ;">' .  $data['order_created'] .'</td>';
                                    echo '<td><button type="button" class="btn waves-effect waves-light btn-outline-info">😢</button></td>';
                                } else if ($data['status'] == 'success') {
                                    echo ' <td> <span class="badge badge-pill badge-success">Success</span> </td>';
                                    // echo '<td style="text-align:center ;">' .  $data['order_created'] .'</td>';
                                    echo '<td> <button  class="btn waves-effect waves-light btn-outline-success">😃</button></td>';
                                }
                                ?>
                ...
                <table class="table table-striped data-table" style="width: 100%; text-align:center;">
                    <h3 style="text-align:center;">Withdrawal Statement</h3>
                    <thead>
                        <tr>
                            <th>Request Id</th>
                            <th>Amount</th>
                    ...
                    <tbody>
                        <?php
                        $sql = "SELECT * FROM `withdraw_balance` where username = '" . $_SESSION['username'] . "' ";
                        $records = mysqli_query($con, $sql); // fetch data from database
                        while ($data = mysqli_fetch_array($records)) {
                        ?>

                            <tr class="active-row" align="center">
                                <td id="soft_name"><?php echo $data['withdraw_id']; ?></td>
                                <td id="soft_name"><?php echo $data['amount']; ?></td>
                                ...
                                if ($data['status'] == 'pending') {
                                    ...
                                    echo ' <td> <span class="badge badge-pill badge-danger">Rejected</span> </td>';
                                    //    echo '<td style="text-align:center ;">' .  $data['order_created'] .'</td>';
                                ...
                                else if ($data['status'] == 'coinfirmed') {
                                    echo ' <td> <span class="badge badge-pill badge-info">Coinfirmed</span> </td>';
                                    // echo '<td style="text-align:center ;">' .  $data['order_created'] .'</td>';
